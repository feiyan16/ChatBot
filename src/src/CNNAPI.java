...
public class CNNAPI {
	...
	public static String URLMaker(String query) {
		...
		if (isSpace == true) {
			...
			URL = "http://newsapi.org/v2/top-headlines?q=" + query_ + "&sources=cnn&apiKey=" + APIKey;
		} else {
			URL = "http://newsapi.org/v2/top-headlines?q=" + query + "&sources=cnn&apiKey=" + APIKey;
		}
		return URL;
	}
	...
	public static String parseNews(String JSON) {
		...
		if (articles.size() != 0) {
			...
			newsOutput = "CNN News: " + title + " Description: " + desc + " To read more, click here: " + url;
		} else
			newsOutput = "There are no headlines in CNN regarding this topic";
		return newsOutput;
