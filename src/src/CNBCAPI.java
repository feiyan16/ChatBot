...
public class CNBCAPI {
	...
	public static String URLMaker(String query) {
		...
		if (isSpace == true) {
			...
			URL = "http://newsapi.org/v2/top-headlines?q=" + query_ + "&sources=cnbc&apiKey=" + APIKey;
		} else {
			URL = "http://newsapi.org/v2/top-headlines?q=" + query + "&sources=cnbc&apiKey=" + APIKey;
		}
		return URL;
	}
	...
	public static String parseNews(String JSON) {
		...
		if (articles.size() != 0) {
			...
			newsOutput = "CNBC News: " + title + " Description: " + desc + " To read more, click here: " + url;
		} else
			newsOutput = "There are no headlines in CNBC regarding this topic";
		return newsOutput;
