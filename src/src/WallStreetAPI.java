...
public class WallStreetAPI {
	...
	public static String URLMaker(String query) {
		...
		if (isSpace == true) {
			...
			URL = "http://newsapi.org/v2/top-headlines?q=" + query_ + "&sources=the-wall-street-journal&apiKey="
					+ APIKey;
		} else {
			URL = "http://newsapi.org/v2/top-headlines?q=" + query + "&sources=the-wall-street-journal&apiKey="
					+ APIKey;
		}
		return URL;
	}
	...
	public static String parseNews(String JSON) {
		...
		if (articles.size() != 0) {
			...
			newsOutput = "The Wall Street Journal: " + title + " Description: " + desc + " To read more, click here: "
					+ url;
		} else
			newsOutput = "There are no headlines in The Wall Street Journal regarding this topic";
		return newsOutput;
