...
public class ABCAPI {
	...
	public static String URLMaker(String query) {
		...
		if (isSpace == true) {
			...
			URL = "http://newsapi.org/v2/top-headlines?q=" + query_ + "&sources=abc-news&apiKey=" + APIKey;
		} else {
			URL = "http://newsapi.org/v2/top-headlines?q=" + query + "&sources=abc-news&apiKey=" + APIKey;
		}
		return URL;
	}
	...
	public static String parseNews(String JSON) {
		...
		if (articles.size() != 0) {
			...
			newsOutput = "ABC News: " + title + " Description: " + desc + " To read more, click here: " + url;
		} else
			newsOutput = "There are no headlines in ABC News regarding this topic";
		return newsOutput;
