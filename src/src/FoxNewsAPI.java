...
public class FoxNewsAPI {
	...
	public static String URLMaker(String query) {
		...
		if (isSpace == true) {
			...
			URL = "http://newsapi.org/v2/top-headlines?q=" + query_ + "&sources=fox-news&apiKey=" + APIKey;
		} else {
			URL = "http://newsapi.org/v2/top-headlines?q=" + query + "&sources=fox-news&apiKey=" + APIKey;
		}
		return URL;
	}
	...
	public static String parseNews(String JSON) {
		...
		if (articles.size() != 0) {
			...
			newsOutput = "Fox News: " + title + " Description: " + desc + " To read more, click here: " + url;
		} else
			newsOutput = "There are no headlines in Fox News regarding this topic";
		return newsOutput;
